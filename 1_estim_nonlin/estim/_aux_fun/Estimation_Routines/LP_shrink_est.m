...
IRF_resp = zeros(IRF_hor,length(responseV));

for jj = 1:length(responseV)
...
rss_cv = locproj_cv(y(:,jj), x, w, H_min, H_max, r, lambdaRange, settings.est.CV_folds); % cross-validated MSE for each value of lambda
...
IRF_resp_aux = locproj(y(:,jj), x, w, H_min, H_max, r, lambda_opt); % IRF to one unit of shock
IRF_resp(:,jj) = IRF_resp_aux; 
end

normalize_index = find(responseV==normalizeV);
...
IRF_normalize = IRF_resp(1,normalize_index);
