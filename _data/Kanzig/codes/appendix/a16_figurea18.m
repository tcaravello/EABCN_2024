...
% This file generates figure A.18
...
smplStart = '1974M01'; 
smplEnd   = '2014M12'; 
% the behavior of the series is subject to debate after 2014, see
% discussion by Hamilton and Kilian. Stop sample before to be on safe side
...
% get alternative series
load('../../data/appendix/dataAppendixM')
% replace activity indicator
data(:,4) = data_extended(:,2);

% number of variables in VAR
nvar = size(data,2);  

% names for paper
varNames_paper = {'Real oil price','World oil production','World oil inventories','Global activity','U.S. industrial production','U.S. CPI'};
varNames_paperVD = {'Real oil price','Oil production','Oil inventories','Global activity','U.S. IP','U.S. CPI'};
...
for j=1:nvar % variable
    ...
    xticks(0:10:horizon)
    if j==1 && includeBase
        legend([p1 p2],{'Kilian index','Baseline'})
    end
end
pause(0.001)
...
tightfig;
if saveFigs
    print('-dpdf', gcf, '../../results/appendix/figurea18');  
end
